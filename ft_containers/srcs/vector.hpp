...
/*   Updated: 2022/11/22 14:24:54 by schuah           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	class vector
	{
		public:
			...
			vector (const vector& x)
			{
				...
				size_type	src_cap = x.capacity();
				if (src_cap <= 0)
					return ;
				this->_start = this->_alloc.allocate(src_cap);
				this->_capacity = this->_start + src_cap;
				this->_end = contruct_from_start(this->_start, x._start, x._end);
			}
		...
		private:
			/* Destroys allocated memory from the start */
